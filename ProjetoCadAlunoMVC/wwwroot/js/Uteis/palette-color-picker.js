    ...
    // Close on clicking outside the palette
      $('body').on(click_handler, function (event) {  
          if (!$(event.target).hasClass(ns + '-button')) {
              if ($($button).find('.' + ns + '-bubble').is(':visible')) {
                  // Função chamada antes de fechar a paleta.
                  if (typeof plugin.settings.onbeforehide_callback === 'function') {
                      plugin.settings.onbeforehide_callback($button);
                  }
              }
              //if ($('.' + ns + '-bubble').is(":visible")) {
              //    debugger;
              //    $('.' + ns + '-bubble:visible').fadeOut(function () {
              //        debugger;
              //    });
